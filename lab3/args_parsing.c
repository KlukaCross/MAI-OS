...
void print_help(char name_program[]) {
    ...
    printf("\t-t\tмаксимальное количество используемой оперативной памяти в байтах для считываемых чисел "
           "(по умолчанию %d). Внимание! Не рекомендуется устанавливать значение меньше %lu*max_threads байт.\n",
           DEFAULT_MAX_MEMORY, MAX_128_HEX_BYTES+sizeof(__int128));
}
    ...
    if (argc == 2)
        *filename = argv[1];
        ...
        if (*max_memory < (MAX_128_HEX_BYTES+sizeof(__int128))*(*max_threads))
            printf("Warning: not recommended to set max_memory less than %lu*max_threads", MAX_128_HEX_BYTES+sizeof(__int128));
