...
bool mqn_init(mq_node *m) {
    if (!(m->context = zmq_ctx_new()))
        return false;
    if (!(m->socket = zmq_socket(m->context, ZMQ_REP)))
        return false;
    m->left_child_address = NULL;
    m->right_child_address = NULL;
    m->left_child_socket = NULL;
    m->right_child_socket = NULL;
    return true;
}
bool mqn_bind(mq_node *m, char* address) {
    return zmq_bind(m->context, address) == 0;
}
bool mqn_connect(mq_node *m, char* address) {
    if (m->left_child_socket == NULL) {
        if (!(m->left_child_socket = zmq_socket(m->context, ZMQ_REQ)))
            return false;
        if (zmq_connect(m->left_child_socket, address) == -1)
            return false;
        m->left_child_address = address;
    } else if (m->right_child_socket == ZMQ_NULL) {
        if (!(m->right_child_socket = zmq_socket(m->context, ZMQ_REQ)))
            return false;
        if (zmq_connect(m->right_child_socket, address) == -1)
            return false;
        m->right_child_address = address;
    } else
        return false;
    return true;
}
...
void mqn_receive(mq_node *m, char** message) { // from parent
    zmq_msg_t req;
    zmq_msg_init(&req);
    zmq_msg_recv(&req, m->socket, 0);
    memcpy(*message, zmq_msg_data(&req), strlen(zmq_msg_data(&req)));
    zmq_msg_close(&req);
}

static char* mqn_request(void* socket, char* message) {
    zmq_msg_t req;
...
char* mqn_left_request(mq_node *m, char* message) { // from children
    if (m->left_child_socket != NULL)
        return mqn_request(m->left_child_socket, message);
    return NULL;
}
char* mqn_right_request(mq_node *m, char* message) { // from children
    if (m->right_child_socket != NULL)
        return mqn_request(m->right_child_socket, message);
    return NULL;
}
